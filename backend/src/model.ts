...
export class OpenAIModel implements ModelAdapter {
  ...
  async chat(request: ChatRequest): Promise<ModelResponse> {
    const response = await this.client.responses.create({
      model: this.model,
      instructions:
        "You are DB, a personal AI assistant. Be helpful, clear, honest, and concise. Never claim to have performed an action you did not perform.",
      input: request.message,
      ...(request.conversationId
        ? { previous_response_id: request.conversationId }
        : {})
    });
